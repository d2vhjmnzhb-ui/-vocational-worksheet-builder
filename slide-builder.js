...
const defaults=()=>({img:'',imagePos:'50% 50%',fade:62,fadeDir:'left'});
function seed(){
...
function slideHTML(s,i){
 ...
 const hasImg=!!s.img;
 return `<div class="slide ${s.type==='cover'?'cover':''} ${hasImg?'has-image':'no-image'}" data-theme="${theme}">
   ...
   <div class="ss-content"><div class="kicker">${esc(s.k)}</div><div class="accent"></div><h${s.type==='cover'?1:2}>${esc(s.t)}</h${s.type==='cover'?1:2}>${body}</div>
   <div class="visual">${hasImg?'':placeholder(s.t)}</div>
...
function render(){
 ...
 $('sstitle').value=s.t;$('ssbodytext').value=s.b;$('sstype').value=s.type;$('sstheme').value=theme;$('ssfade').value=s.fade??62;$('ssfadeValue').textContent=(s.fade??62)+'%';$('ssfadeDir').value=s.fadeDir||'left';
 $('ssimgState').textContent=s.img?'มีภาพในสไลด์นี้':'ยังไม่มีภาพ';
}
function open(){
...
document.addEventListener('DOMContentLoaded',()=>{
 document.body.insertAdjacentHTML('beforeend',`<div id="slideStudio">
  ...
  <div class="ssbody">
   ...
   <main class="sscenter"><div class="sseditbar"><span>สไลด์ปัจจุบัน</span><button id="ssquickUpload">▣ เพิ่มภาพ</button><button id="ssquickGoogle">G ค้น Google</button><button id="ssquickAI">✦ AI สร้างภาพ</button></div><div class="sscanvas" id="sscanvas"></div><div class="sseditpanel"><div><label>หัวข้อหลัก</label><input id="sstitle"></div><div><label>เนื้อหา</label><textarea id="ssbodytext"></textarea></div><div><label>รูปแบบ</label><select id="sstype"><option value="cover">หน้าปก</option><option value="visual">ข้อความ + ภาพ</option><option value="bullets">หัวข้อย่อย</option><option value="cards">การ์ดข้อมูล</option></select></div><button class="ssapply" id="ssapply">อัปเดตสไลด์</button></div></main>
   <aside class="ssright">
