...
function readQuestions(){
  ...
  document.querySelectorAll('#paper .exercise > li').forEach(li=>{
    ...
    const rawAnswer=txt(li.querySelector('.answer'));
    if(choices.length===4){
      const m=rawAnswer.match(/(?:เฉลย(?:\/แนวคำตอบ)?|คำตอบ)\s*[:：]?\s*([กขคง])(?:\s*[.)])?/i);
      if(!m)return;
      out.push({type:'mcq',q,choices:{ก:choices[0],ข:choices[1],ค:choices[2],ง:choices[3]},answer:m[1],points:1});
    }else{
      const modelAnswer=cleanAnswerText(rawAnswer)||'—';
      out.push({type:'text',q,modelAnswer,points:1});
    }
  });
  return out;
}
